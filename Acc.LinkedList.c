...
rmfromlist(REC *r, rectype t) {
    ...
    if (first == NULL) /*we do not have any records added yet*/
        return false;
    ...
    if (reccmp(first->data,r->data) == 0) {
        ...
        free(r->data.type);
        if (r->data.descr != NULL) //beacuse it is optional
            free(r->data.descr);
        free(r);
...
clearlist(rectype r) {
    ...
    while (rp != NULL) {
        ...
        free(rp->data.type);
        if (rp->data.descr != NULL) //beacuse it is optional
            free(rp->data.descr);
        free(rp);
        rp = nx;
    }
...
Index
count_filtered(rectype t, bool (*func)(recdata r)) {
    Index c = 0;
    REC *rp = getsrc(t);

    while (rp != NULL) {
        /*count only recods that can pass the filter*/
        if ((*func)(rp->data) ==true)
            c++;
...
REC *
jump(REC **start, int num_records, rectype t) {
    Index c = 1;
    if (*start == NULL)
        *start = getsrc(t);
...
int traversen(void(*func)(recdata rp),rectype t, int max, bool firstpage) {
    ...
    switch (t){
        case income:
            /*first show, initialize from source*/
            if (lastTraversenI == NULL)
                lastTraversenI = getsrc(t);

            /* on first show after app load restore
               state from prev viewing if  showlast:... feature
               is enabled in config file */
            if (firstpage && lastTraversenIb != NULL)
                iterator = lastTraversenIb;
            else{
                iterator = lastTraversenI;
                lastTraversenIb = lastTraversenI; //used for showlast:... feature
            }
        break;
        case outcome:
...
int reccmp(recdata r1, recdata r2) {
    ...
    if (r1.descr!= NULL && r2.descr != NULL)
